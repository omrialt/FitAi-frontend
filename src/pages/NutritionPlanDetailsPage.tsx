            ...
            <section className="bg-surface-container-lowest rounded-xl p-8 border border-outline-variant/10 mb-6 text-center">
              ...
              <div className="flex items-center justify-center gap-10 mt-6 flex-wrap">
                {[
                  ...
                  // Screen 14 keys the macros to the chart theme: protein on
                  // chart-1, carbs on chart-2, fats on chart-4. `secondary` is
                  // chart-2; `on-secondary-container` was the fill meant for
                  // text sitting *inside* a secondary chip, so it read too dark
                  // in light mode for a standalone figure.
                  { label: t('nutrition.carbs'), value: totalC, color: 'text-secondary' },
                  { label: t('nutrition.fats'), value: totalF, color: 'text-warning' },
                ].map((m) => (
